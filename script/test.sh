...
for TEST_FILE in "$TESTS_DIR"/*.jiang; do
    ...
    if [[ "$BASENAME" == fail_* ]]; then
        if grep -qiE "Semantic Error|failed due to" "$BUILD_DIR/compiler_output.log"; then
    ...
    # 针对正常的正确用例
        ...
        if [ ! -f "$OUTPUT_C" ]; then
            ...
            # 尝试编译生成的 C 代码
            if gcc "$OUTPUT_C" -o "$OUTPUT_BIN" > "$BUILD_DIR/gcc_output.log" 2>&1; then
