...
OUT_PRIVATE_INSTANCE_METHOD_LOG="$BUILD_DIR/stage2_llvm_invalid_private_instance_method.log"
OUT_PRIVATE_STATIC_METHOD_LOG="$BUILD_DIR/stage2_llvm_invalid_private_static_method.log"
...
OUT_OPTIONAL_COALESCE_NON_OPTIONAL_LOG="$BUILD_DIR/stage2_llvm_invalid_optional_coalesce_non_optional.log"
OUT_OPTIONAL_COALESCE_FALLBACK_TYPE_LOG="$BUILD_DIR/stage2_llvm_invalid_optional_coalesce_fallback_type.log"
OUT_OPTIONAL_COALESCE_IMPURE_LEFT_LOG="$BUILD_DIR/stage2_llvm_invalid_optional_coalesce_impure_left.log"
...
OUT_UNION_INSTANCE_CALL_THROUGH_TYPE_LOG="$BUILD_DIR/stage2_llvm_invalid_union_instance_call_through_type.log"
OUT_UNION_STATIC_CALL_THROUGH_INSTANCE_LOG="$BUILD_DIR/stage2_llvm_invalid_union_static_call_through_instance.log"
OUT_ENUM_INSTANCE_CALL_THROUGH_TYPE_LOG="$BUILD_DIR/stage2_llvm_invalid_enum_instance_call_through_type.log"
OUT_ENUM_STATIC_CALL_THROUGH_INSTANCE_LOG="$BUILD_DIR/stage2_llvm_invalid_enum_static_call_through_instance.log"
...
set +e
"$BUILD_DIR/stage2c" --emit-llvm "$PROJECT_ROOT/compiler/tests/samples/invalid_optional_coalesce_non_optional.jiang" > "$OUT_OPTIONAL_COALESCE_NON_OPTIONAL_LOG"
STATUS=$?
set -e

if [[ $STATUS -eq 0 ]]; then
    echo "stage2 llvm error smoke expected invalid_optional_coalesce_non_optional to fail" >&2
    exit 1
fi

if [[ "$(<"$OUT_OPTIONAL_COALESCE_NON_OPTIONAL_LOG")" != *"null coalescing requires optional left operand"* ]]; then
    echo "stage2 llvm error smoke missing optional coalesce left operand diagnostic" >&2
    exit 1
fi

if rg -q '^; ModuleID = ' "$OUT_OPTIONAL_COALESCE_NON_OPTIONAL_LOG"; then
    echo "stage2 llvm error smoke unexpectedly produced llvm ir for invalid_optional_coalesce_non_optional" >&2
    exit 1
fi

set +e
"$BUILD_DIR/stage2c" --emit-llvm "$PROJECT_ROOT/compiler/tests/samples/invalid_optional_coalesce_fallback_type.jiang" > "$OUT_OPTIONAL_COALESCE_FALLBACK_TYPE_LOG"
STATUS=$?
set -e

if [[ $STATUS -eq 0 ]]; then
    echo "stage2 llvm error smoke expected invalid_optional_coalesce_fallback_type to fail" >&2
    exit 1
fi

if [[ "$(<"$OUT_OPTIONAL_COALESCE_FALLBACK_TYPE_LOG")" != *"null coalescing fallback type mismatch"* ]]; then
    echo "stage2 llvm error smoke missing optional coalesce fallback diagnostic" >&2
    exit 1
fi

if rg -q '^; ModuleID = ' "$OUT_OPTIONAL_COALESCE_FALLBACK_TYPE_LOG"; then
    echo "stage2 llvm error smoke unexpectedly produced llvm ir for invalid_optional_coalesce_fallback_type" >&2
    exit 1
fi

set +e
"$BUILD_DIR/stage2c" --emit-llvm "$PROJECT_ROOT/compiler/tests/samples/invalid_optional_coalesce_impure_left.jiang" > "$OUT_OPTIONAL_COALESCE_IMPURE_LEFT_LOG"
STATUS=$?
set -e

if [[ $STATUS -eq 0 ]]; then
    echo "stage2 llvm error smoke expected invalid_optional_coalesce_impure_left to fail" >&2
    exit 1
fi

if [[ "$(<"$OUT_OPTIONAL_COALESCE_IMPURE_LEFT_LOG")" != *"null coalescing currently requires simple left expression"* ]]; then
    echo "stage2 llvm error smoke missing optional coalesce impure left diagnostic" >&2
    exit 1
fi

if rg -q '^; ModuleID = ' "$OUT_OPTIONAL_COALESCE_IMPURE_LEFT_LOG"; then
    echo "stage2 llvm error smoke unexpectedly produced llvm ir for invalid_optional_coalesce_impure_left" >&2
    exit 1
fi
...
set +e
"$BUILD_DIR/stage2c" --emit-llvm "$PROJECT_ROOT/compiler/tests/samples/invalid_optional_chain_impure_member_base.jiang" > "$OUT_OPTIONAL_CHAIN_IMPURE_MEMBER_BASE_LOG"
STATUS=$?
set -e

if [[ $STATUS -eq 0 ]]; then
    echo "stage2 llvm error smoke expected invalid_optional_chain_impure_member_base to fail" >&2
    exit 1
fi

if [[ "$(<"$OUT_OPTIONAL_CHAIN_IMPURE_MEMBER_BASE_LOG")" != *"optional chain currently requires simple base expression"* ]]; then
    echo "stage2 llvm error smoke missing optional chain impure member base diagnostic" >&2
    exit 1
fi
...
set +e
"$BUILD_DIR/stage2c" --emit-llvm "$PROJECT_ROOT/compiler/tests/samples/invalid_optional_switch_non_exhaustive.jiang" > "$OUT_OPTIONAL_SWITCH_NON_EXHAUSTIVE_LOG"
STATUS=$?
set -e

if [[ $STATUS -eq 0 ]]; then
    echo "stage2 llvm error smoke expected invalid_optional_switch_non_exhaustive to fail" >&2
    exit 1
fi

if [[ "$(<"$OUT_OPTIONAL_SWITCH_NON_EXHAUSTIVE_LOG")" != *"non-exhaustive switch"* ]]; then
    echo "stage2 llvm error smoke missing optional switch non-exhaustive diagnostic" >&2
    exit 1
fi
...
set +e
"$BUILD_DIR/stage2c" --emit-llvm "$PROJECT_ROOT/compiler/tests/samples/invalid_union_instance_call_through_type.jiang" > "$OUT_UNION_INSTANCE_CALL_THROUGH_TYPE_LOG"
STATUS=$?
set -e

if [[ $STATUS -eq 0 ]]; then
    echo "stage2 llvm error smoke expected invalid_union_instance_call_through_type to fail" >&2
    exit 1
fi

if [[ "$(<"$OUT_UNION_INSTANCE_CALL_THROUGH_TYPE_LOG")" != *"instance method called through type"* ]]; then
    echo "stage2 llvm error smoke missing union instance-through-type diagnostic" >&2
    exit 1
fi

if rg -q '^; ModuleID = ' "$OUT_UNION_INSTANCE_CALL_THROUGH_TYPE_LOG"; then
    echo "stage2 llvm error smoke unexpectedly produced llvm ir for invalid_union_instance_call_through_type" >&2
    exit 1
fi

set +e
"$BUILD_DIR/stage2c" --emit-llvm "$PROJECT_ROOT/compiler/tests/samples/invalid_union_static_call_through_instance.jiang" > "$OUT_UNION_STATIC_CALL_THROUGH_INSTANCE_LOG"
STATUS=$?
set -e

if [[ $STATUS -eq 0 ]]; then
    echo "stage2 llvm error smoke expected invalid_union_static_call_through_instance to fail" >&2
    exit 1
fi

if [[ "$(<"$OUT_UNION_STATIC_CALL_THROUGH_INSTANCE_LOG")" != *"static function called through instance"* ]]; then
    echo "stage2 llvm error smoke missing union static-through-instance diagnostic" >&2
    exit 1
fi

if rg -q '^; ModuleID = ' "$OUT_UNION_STATIC_CALL_THROUGH_INSTANCE_LOG"; then
    echo "stage2 llvm error smoke unexpectedly produced llvm ir for invalid_union_static_call_through_instance" >&2
    exit 1
fi

set +e
"$BUILD_DIR/stage2c" --emit-llvm "$PROJECT_ROOT/compiler/tests/samples/invalid_enum_instance_call_through_type.jiang" > "$OUT_ENUM_INSTANCE_CALL_THROUGH_TYPE_LOG"
STATUS=$?
set -e

if [[ $STATUS -eq 0 ]]; then
    echo "stage2 llvm error smoke expected invalid_enum_instance_call_through_type to fail" >&2
    exit 1
fi

if [[ "$(<"$OUT_ENUM_INSTANCE_CALL_THROUGH_TYPE_LOG")" != *"instance method called through type"* ]]; then
    echo "stage2 llvm error smoke missing enum instance-through-type diagnostic" >&2
    exit 1
fi

if rg -q '^; ModuleID = ' "$OUT_ENUM_INSTANCE_CALL_THROUGH_TYPE_LOG"; then
    echo "stage2 llvm error smoke unexpectedly produced llvm ir for invalid_enum_instance_call_through_type" >&2
    exit 1
fi

set +e
"$BUILD_DIR/stage2c" --emit-llvm "$PROJECT_ROOT/compiler/tests/samples/invalid_enum_static_call_through_instance.jiang" > "$OUT_ENUM_STATIC_CALL_THROUGH_INSTANCE_LOG"
STATUS=$?
set -e

if [[ $STATUS -eq 0 ]]; then
    echo "stage2 llvm error smoke expected invalid_enum_static_call_through_instance to fail" >&2
    exit 1
fi

if [[ "$(<"$OUT_ENUM_STATIC_CALL_THROUGH_INSTANCE_LOG")" != *"static function called through instance"* ]]; then
    echo "stage2 llvm error smoke missing enum static-through-instance diagnostic" >&2
    exit 1
fi

if rg -q '^; ModuleID = ' "$OUT_ENUM_STATIC_CALL_THROUGH_INSTANCE_LOG"; then
    echo "stage2 llvm error smoke unexpectedly produced llvm ir for invalid_enum_static_call_through_instance" >&2
    exit 1
fi

set +e
"$BUILD_DIR/stage2c" --emit-llvm "$PROJECT_ROOT/compiler/tests/samples/invalid_import_private_instance_method.jiang" > "$OUT_PRIVATE_INSTANCE_METHOD_LOG"
STATUS=$?
set -e

if [[ $STATUS -eq 0 ]]; then
    echo "stage2 llvm error smoke expected invalid_import_private_instance_method to fail" >&2
    exit 1
fi

if [[ "$(<"$OUT_PRIVATE_INSTANCE_METHOD_LOG")" != *"method is private"* ]]; then
    echo "stage2 llvm error smoke missing private instance method diagnostic" >&2
    exit 1
fi

if rg -q '^; ModuleID = ' "$OUT_PRIVATE_INSTANCE_METHOD_LOG"; then
    echo "stage2 llvm error smoke unexpectedly produced llvm ir for invalid_import_private_instance_method" >&2
    exit 1
fi

set +e
"$BUILD_DIR/stage2c" --emit-llvm "$PROJECT_ROOT/compiler/tests/samples/invalid_import_private_static_method.jiang" > "$OUT_PRIVATE_STATIC_METHOD_LOG"
STATUS=$?
set -e

if [[ $STATUS -eq 0 ]]; then
    echo "stage2 llvm error smoke expected invalid_import_private_static_method to fail" >&2
    exit 1
fi

if [[ "$(<"$OUT_PRIVATE_STATIC_METHOD_LOG")" != *"method is private"* ]]; then
    echo "stage2 llvm error smoke missing private static method diagnostic" >&2
    exit 1
fi

if rg -q '^; ModuleID = ' "$OUT_PRIVATE_STATIC_METHOD_LOG"; then
    echo "stage2 llvm error smoke unexpectedly produced llvm ir for invalid_import_private_static_method" >&2
    exit 1
fi
