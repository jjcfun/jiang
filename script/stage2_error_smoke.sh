...
OUT_STRUCT_METHOD_MISPARSE_LOG="$BUILD_DIR/stage2_invalid_struct_method_before_feature_misparse.log"
...
OUT_OPTIONAL_COALESCE_NON_OPTIONAL_LOG="$BUILD_DIR/stage2_invalid_optional_coalesce_non_optional.log"
OUT_OPTIONAL_COALESCE_FALLBACK_TYPE_LOG="$BUILD_DIR/stage2_invalid_optional_coalesce_fallback_type.log"
OUT_OPTIONAL_COALESCE_IMPURE_LEFT_LOG="$BUILD_DIR/stage2_invalid_optional_coalesce_impure_left.log"
...
set +e
"$BUILD_DIR/stage2c" "$PROJECT_ROOT/compiler/tests/samples/invalid_struct_static_uses_self.jiang" > "$OUT_STRUCT_STATIC_USES_SELF_LOG"
STATUS=$?
set -e

if [[ $STATUS -eq 0 ]]; then
    echo "stage2 error smoke expected invalid_struct_static_uses_self to fail" >&2
    exit 1
fi

if [[ "$(<"$OUT_STRUCT_STATIC_USES_SELF_LOG")" != *"self used in static function"* ]]; then
    echo "stage2 error smoke missing static self diagnostic" >&2
    exit 1
fi

set +e
"$BUILD_DIR/stage2c" "$PROJECT_ROOT/compiler/tests/samples/invalid_struct_instance_call_through_type.jiang" > "$OUT_STRUCT_INSTANCE_CALL_THROUGH_TYPE_LOG"
STATUS=$?
set -e

if [[ $STATUS -eq 0 ]]; then
    echo "stage2 error smoke expected invalid_struct_instance_call_through_type to fail" >&2
    exit 1
fi

if [[ "$(<"$OUT_STRUCT_INSTANCE_CALL_THROUGH_TYPE_LOG")" != *"instance method called through type"* ]]; then
    echo "stage2 error smoke missing instance-through-type diagnostic" >&2
    exit 1
fi

set +e
"$BUILD_DIR/stage2c" "$PROJECT_ROOT/compiler/tests/samples/invalid_struct_static_call_through_instance.jiang" > "$OUT_STRUCT_STATIC_CALL_THROUGH_INSTANCE_LOG"
STATUS=$?
set -e

if [[ $STATUS -eq 0 ]]; then
    echo "stage2 error smoke expected invalid_struct_static_call_through_instance to fail" >&2
    exit 1
fi
...
set +e
"$BUILD_DIR/stage2c" "$PROJECT_ROOT/compiler/tests/samples/invalid_struct_duplicate_method.jiang" > "$OUT_STRUCT_DUPLICATE_METHOD_LOG"
STATUS=$?
set -e

if [[ $STATUS -eq 0 ]]; then
    echo "stage2 error smoke expected invalid_struct_duplicate_method to fail" >&2
    exit 1
fi

if [[ "$(<"$OUT_STRUCT_DUPLICATE_METHOD_LOG")" != *"duplicate method"* ]]; then
    echo "stage2 error smoke missing duplicate method diagnostic" >&2
    exit 1
fi

set +e
"$BUILD_DIR/stage2c" "$PROJECT_ROOT/compiler/tests/samples/invalid_struct_field_method_name_conflict.jiang" > "$OUT_STRUCT_FIELD_METHOD_NAME_CONFLICT_LOG"
STATUS=$?
set -e

if [[ $STATUS -eq 0 ]]; then
    echo "stage2 error smoke expected invalid_struct_field_method_name_conflict to fail" >&2
    exit 1
fi

if [[ "$(<"$OUT_STRUCT_FIELD_METHOD_NAME_CONFLICT_LOG")" != *"field and method names must not conflict"* ]]; then
    echo "stage2 error smoke missing field/method conflict diagnostic" >&2
    exit 1
fi

set +e
"$BUILD_DIR/stage2c" "$PROJECT_ROOT/compiler/tests/samples/invalid_struct_static_init.jiang" > "$OUT_STRUCT_STATIC_INIT_LOG"
STATUS=$?
set -e

if [[ $STATUS -eq 0 ]]; then
    echo "stage2 error smoke expected invalid_struct_static_init to fail" >&2
    exit 1
fi

if [[ "$(<"$OUT_STRUCT_STATIC_INIT_LOG")" != *"static init is invalid"* ]]; then
    echo "stage2 error smoke missing static init diagnostic" >&2
    exit 1
fi

set +e
"$BUILD_DIR/stage2c" "$PROJECT_ROOT/compiler/tests/samples/invalid_struct_method_before_feature_misparse.jiang" > "$OUT_STRUCT_METHOD_MISPARSE_LOG"
STATUS=$?
set -e

if [[ $STATUS -eq 0 ]]; then
    echo "stage2 error smoke expected invalid_struct_method_before_feature_misparse to fail" >&2
    exit 1
fi

if [[ "$(<"$OUT_STRUCT_METHOD_MISPARSE_LOG")" != *"unexpected token"* ]]; then
    echo "stage2 error smoke missing struct method parse diagnostic" >&2
    exit 1
fi
...
set +e
"$BUILD_DIR/stage2c" "$PROJECT_ROOT/compiler/tests/samples/invalid_optional_coalesce_non_optional.jiang" > "$OUT_OPTIONAL_COALESCE_NON_OPTIONAL_LOG"
STATUS=$?
set -e

if [[ $STATUS -eq 0 ]]; then
    echo "stage2 error smoke expected invalid_optional_coalesce_non_optional to fail" >&2
    exit 1
fi

if [[ "$(<"$OUT_OPTIONAL_COALESCE_NON_OPTIONAL_LOG")" != *"null coalescing requires optional left operand"* ]]; then
    echo "stage2 error smoke missing optional coalesce left operand diagnostic" >&2
    exit 1
fi

set +e
"$BUILD_DIR/stage2c" "$PROJECT_ROOT/compiler/tests/samples/invalid_optional_coalesce_fallback_type.jiang" > "$OUT_OPTIONAL_COALESCE_FALLBACK_TYPE_LOG"
STATUS=$?
set -e

if [[ $STATUS -eq 0 ]]; then
    echo "stage2 error smoke expected invalid_optional_coalesce_fallback_type to fail" >&2
    exit 1
fi

if [[ "$(<"$OUT_OPTIONAL_COALESCE_FALLBACK_TYPE_LOG")" != *"null coalescing fallback type mismatch"* ]]; then
    echo "stage2 error smoke missing optional coalesce fallback diagnostic" >&2
    exit 1
fi

set +e
"$BUILD_DIR/stage2c" "$PROJECT_ROOT/compiler/tests/samples/invalid_optional_coalesce_impure_left.jiang" > "$OUT_OPTIONAL_COALESCE_IMPURE_LEFT_LOG"
STATUS=$?
set -e

if [[ $STATUS -eq 0 ]]; then
    echo "stage2 error smoke expected invalid_optional_coalesce_impure_left to fail" >&2
    exit 1
fi

if [[ "$(<"$OUT_OPTIONAL_COALESCE_IMPURE_LEFT_LOG")" != *"null coalescing currently requires simple left expression"* ]]; then
    echo "stage2 error smoke missing optional coalesce impure left diagnostic" >&2
    exit 1
fi
...
set +e
"$BUILD_DIR/stage2c" "$PROJECT_ROOT/compiler/tests/samples/invalid_optional_chain_impure_member_base.jiang" > "$OUT_OPTIONAL_CHAIN_IMPURE_MEMBER_BASE_LOG"
STATUS=$?
set -e

if [[ $STATUS -eq 0 ]]; then
    echo "stage2 error smoke expected invalid_optional_chain_impure_member_base to fail" >&2
    exit 1
fi

if [[ "$(<"$OUT_OPTIONAL_CHAIN_IMPURE_MEMBER_BASE_LOG")" != *"optional chain currently requires simple base expression"* ]]; then
    echo "stage2 error smoke missing optional chain impure member base diagnostic" >&2
    exit 1
fi

set +e
"$BUILD_DIR/stage2c" "$PROJECT_ROOT/compiler/tests/samples/invalid_optional_pattern_non_optional.jiang" > "$OUT_OPTIONAL_PATTERN_NON_OPTIONAL_LOG"
STATUS=$?
set -e

if [[ $STATUS -eq 0 ]]; then
    echo "stage2 error smoke expected invalid_optional_pattern_non_optional to fail" >&2
    exit 1
fi

if [[ "$(<"$OUT_OPTIONAL_PATTERN_NON_OPTIONAL_LOG")" != *"union pattern requires union operand"* ]]; then
    echo "stage2 error smoke missing optional pattern non-optional diagnostic" >&2
    exit 1
fi

set +e
"$BUILD_DIR/stage2c" "$PROJECT_ROOT/compiler/tests/samples/invalid_optional_pattern_label.jiang" > "$OUT_OPTIONAL_PATTERN_LABEL_LOG"
STATUS=$?
set -e

if [[ $STATUS -eq 0 ]]; then
    echo "stage2 error smoke expected invalid_optional_pattern_label to fail" >&2
    exit 1
fi
