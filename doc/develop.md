...
*   部分对齐：
    ...
    *   枚举底层类型、`.ok` 一类简写
    *   `union`：当前已支持最小声明、构造、payload binding 和基于 tag 的 `switch`，尚未覆盖简写构造、多值解构与更完整布局语义
    *   更完整模块语义
*   仍未开始：
    *   类型转换
    *   元组
    *   `switch`：当前已支持最小 Int / enum / union case、union payload binding 与 `else:`，尚未覆盖更完整 pattern / 穷尽性检查
    *   模式匹配 / binding
    *   泛型
    *   `async`
    *   FFI 作为正式用户语法
