...
*   `compiler/` 这个目录名是长期稳定名称
*   但当前仓库中 `compiler/` 里的实现仍属于 Stage1 后期主线，用于孵化下一代 Jiang 编译器骨架

也就是说，当前 `compiler/` 还不是 README 定义里的正式 Stage2。真正的 Stage2 应建立在这条主线已经更成熟之后。
...
### 当前优先主线：Stage1 默认 LLVM 后端评估 + `compiler/` 骨架孵化
...
`compiler/` 当前阶段的目录/阶段说明见 `doc/plan_stage2_start.md`，但它仍应被视为 Stage1 后期主线，而不是正式 Stage2。
