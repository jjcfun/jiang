...
*   部分对齐：
    ...
    *   指针自动解引用 / 所有权相关语义
    *   `T[]` 的完整泛化
    *   `T?` / `T!`：当前已支持最小 optional / mutable qualifier 语法、`null`、`== null` / `!= null` 判空、`Option.some(_ x)` 判空解包、`?.field` / `?[index]` optional chain、`??` 默认值、基础 expected-type / 推断传播、`Int?[2][3]` / `Int[2]!` 这类后缀组合、union pattern 的 `_!` mutable binding，以及 struct init 中省略 optional 字段默认 `null` 与 C/LLVM codegen；但 `!` 仍未收紧赋值约束
