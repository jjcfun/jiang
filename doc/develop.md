...
*   部分对齐：
    ...
    *   指针自动解引用 / 所有权相关语义
    *   `T[]` 的完整泛化
    *   `T?` / `T!`：当前已支持最小 optional / mutable qualifier 语法、`null`、基础 expected-type / 推断传播、`Int?[2][3]` / `Int[2]!` 这类后缀组合、以及 union pattern 的 `_!` mutable binding 与 C/LLVM codegen；但 `!` 仍未收紧赋值约束
