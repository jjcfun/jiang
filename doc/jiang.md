...
`@region(anchor, value: T = anchor)` 中的 `value` shape 显式来自类型泛型参数 `T`。
字段省略 `value` binding 时，`anchor` 会填充该 shape 的全部 slots；显式绑定时仍必须提供
完整 shape。省略 `= anchor` 后不提供默认 binding。region slot 不能与同一 nominal 的字段、
泛型参数或其他 region slot 重名；函数、内部类型和 extension 成员不在该冲突域。

reference 的完整 Shape 是 `reference(shape(T))`，概念 schema 为
`@region(r, value: T = r)`；外层 borrow 与 pointee Shape 是两个逻辑
...
跨 Domain，并继续由普通 lifetime/borrow check 约束。
...
- `T&` 可以在 `T: Sendable` 时作为参数、result 或 capture 跨 Domain。引用可以保存或返回，但不能
  活过来源 owner；借用存活期间 owner 仍不能被 move 或 drop。
- `T&!` 不直接跨 Domain；需要共享引用时可以自动 reborrow 为 `T&`，并在 shared reborrow 存活期间
  冻结原 `T&!`。
...
ownership 或 lifetime 保证；这些保证由 handle 的实现负责。

domain-bound owned closure 也遵守同一规则。创建 `Fn<async [domain] (...)>^` 时，值 capture 必须能
以对应的 move/copy 方式安全进入目标 Domain；`T&` capture 由 closure environment 的 lifetime shape
传播，不能活过来源 owner。
...
`#package` 的字段之间，以及 `dependencies`、`lang`、`generate` 子块的字段之间，可以放置
`#doc` 或其他返回 Attribute 的语言块。包字段不是独立声明节点，这些 Attribute 被接受但不附着，
也不改变 `info` 的配置值；子块末尾的 Attribute 同样允许忽略。需要记录整个包声明的文档时，
将 `#doc` 放在 `#package` 外部。字段位置的子语言若返回表达式、声明或成员等其他类别，会报诊断；
放在字段右值中的子语言仍按普通表达式解析。
