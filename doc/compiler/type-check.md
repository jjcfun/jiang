...
type check 消费 resolved HIR，输出 `TypeCheckResults`。它负责类型引用解析、function
...
- `ResolveStore` / `DefTable` / namespace facts：名字和 definition 事实。
- `TypeTable`：类型驻留和 builtin type。
...
`TypeCheckResults` 是 MIR、layout、borrow check 和 backend 的共享输入：
...
普通 definition 的类型结果写入 `TypeCheckResults`，不写回 `TypeTable.def_types`。
...
- `TypeCheckResults` 可以在 lowering/backend 完成后释放；长期增量再引入版本化缓存策略。
