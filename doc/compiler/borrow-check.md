...
借用检查按函数及其依赖推进，不要求先生成整个程序的 JIL；本次检查所需的语义与布局事实必须完整。
最终检查补齐其余可达代码，并复用先前结果；代码或依赖事实变化后不能继续沿用旧的检查结论。

无环 CFG 按拓扑顺序检查，合流点先收齐所有前驱状态，处理后即可释放其输入状态。
含环 CFG 使用固定点迭代，保留后续迭代需要的输入状态；两种调度共用相同的转移与合流规则。

编译期表达式不能继承所在运行期函数的参数或局部变量值。编译期执行中的局部变量必须在该次
执行中完成初始化，其借用、移动和析构遵守与普通代码一致的安全规则。
