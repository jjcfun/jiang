...
也不负责证明泛型约束正确；它只从 HIR reachable tree 和 `TypeCheckStore` 中收集需要生成的
concrete instances。
...
- 为 MIR lowering 提供 `MonomorphStore`。
...
`SubstitutionMap` 不放在 `TypeCheckStore` 中。它是 monomorph/MIR lowering 针对单个 concrete
instance 的临时上下文，生命周期短于 type check 全局结果。

## 边界

- generic template function 不直接生成 MIR body。
- monomorph 不复制 HIR，不修改 `TypeCheckStore`。
...
- 所有 concrete MIR function 都应来自 `MonomorphStore` 或非泛型 reachable function。
