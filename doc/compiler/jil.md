...
JIL 按函数或编译期表达式及其依赖生成。Sema 可以在 source graph 尚未封闭时请求 JIL 求值，
但执行入口及其调用依赖必须先通过相应的语义检查、借用检查和析构处理。
最终编译复用按需生成的结果，并补查全部可达代码。仅用于编译期求值的入口不作为运行期入口发射。

生成函数和静态表的存在不表示它们需要运行期发射。运行期入口及其函数引用、静态初始化器引用、
协程和 executor 隐含依赖共同决定发射集合；仅用于求值的缓存不属于这个集合。随后出现运行期
需求时，可以接纳已有结果并继续闭合依赖，不必重新生成函数或静态表。

## 编译期执行

eval、generate 和普通编译期值需求共用 JIL 执行语义。执行器消费已经确定的类型、调用目标和转换，
不重新解释 AST 或重复进行重载决议。编译期运算采用目标平台的数值及布局规则，而不是宿主平台规则。

编译期局部存储遵守初始化、值复制、借用、移动与析构语义；编译器回收存储不能替代语言析构。
求值结果必须能表示为合法的编译期值，不能将宿主地址泄漏到目标程序。执行受步骤与递归配额约束，
非法操作或超出执行环境能力的操作必须诊断，不能隐式调用宿主 FFI 或切换执行模式。

执行器在语句临时值、局部存储与调用帧结束时回收空间；返回值转交调用者，但借用不会延长目标的
生命周期。槽位复用必须区分不同次分配，不能使旧引用重新有效。显式堆对象由 JIL 的释放操作结束
生命周期，共享不可变常量复用只读存储。存储预算按保留容量及管理元数据计量，与执行步骤配额独立。
这些存储策略不改变 JIL、借用检查或析构语义，也不要求增加通用优化 pass。
