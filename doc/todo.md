...
- [x] 删除 `lower_jir.jiang` 中扫描式 type decl 查询：
  - [x] `type_decl_for_binding`
  - [x] `type_decl_for_symbol`
  - [x] `type_decl_id_for_symbol`
- [x] 将 field/variant metadata 查询改为：
  - [x] `TypeId -> TypeInfo -> DeclId`
  - [x] codegen 中 `DeclId -> JirDeclRef -> JirTypeDecl`
  - [x] lowering 构建当前模块时减少本地 `DeclId` 线性扫描。
- [x] 删除 codegen 中 nominal type 的 binding fallback：
  - [x] `type_decl_for_binding`
...
- [x] 增加 AST 声明查询 helper：
  - [x] `module_file(ModuleId) -> AstFile`
  - [x] `decl_ast(DeclId) -> AstDecl`
  - [x] `decl_binding(DeclId) -> Binding`
- [x] 重构 `type_check.decl_for_binding`：
  - [x] 优先通过 `DeclInfo.decl_id/module_id/ast_index` 查询。
  - [x] 删除对当前文件 top-level binding 的线性扫描依赖。
- [x] 将字段、method、union/enum 查询逐步改成 `DeclId` 驱动。
- [x] 保留本地变量和表达式 span lookup，暂不混入全局 decl_table。
