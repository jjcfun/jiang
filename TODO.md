...
- [ ] 类型修饰：已支持最小 `T?` / `null` / `T!` 语法、可选值 codegen、`== null` / `!= null` 判空、`Option.some(_ x)` 判空解包、`?.field` / `?[index]` optional chain、`??` 默认值、基础推断传播、`Int?[2][3]` / `Int[2]!` 这类后缀组合、union pattern `_!` mutable binding，以及 struct init 中省略 optional 字段默认 `null`；但 `!` 仍未收紧赋值规则
