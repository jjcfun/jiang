...
- [ ] `union`：已支持最小声明、构造、payload binding 与基于 tag 的 `switch`，尚未对齐简写构造、多值解构与更完整布局语义
...
- [ ] 类型转换
- [ ] 元组
- [ ] `switch`：已支持最小 Int / enum / union case、union payload binding 与 `else:`，尚未对齐更完整 pattern / 穷尽性检查
- [ ] 模式匹配 / binding
- [ ] 泛型
- [ ] `async`
