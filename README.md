...
+ **Stage2**: 在 Stage1 自举编译器真正收口并可接管主职责之后，再用 Jiang 语言重构 Jiang 编译器，并实现自定义语法等高级功能。该阶段当前已经完成首个正式主线目标：`compiler/` 是唯一继续演进的编译器主线目录，默认 `script/build_stage2.sh` 已采用“Stage1 引导 -> Stage2 自重编”的两段式自举构建链，且已具备 `frontend -> HIR -> JIR -> C/LLVM`、多模块与 `public`/alias import、`struct` / `enum` / `UInt8` / `UInt8[]` / array / pointer 基础语义，以及 `emit-c` / `run` / `selfhost` / `error` / `llvm` / `llvm-error` / `complete` 回归；`bootstrap/` 继续保留为已完成的 Stage1 冻结基线
