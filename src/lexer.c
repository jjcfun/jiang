...
static Token ident_or_keyword(Lexer* lexer) {
    ...
    if (length == 6 && strncmp(lexer->start, "extern", 6) == 0) {
        return make_token(lexer, TOKEN_KW_EXTERN, lexer->start, length);
    }
    ...
    if (length == 6 && strncmp(lexer->start, "record", 6) == 0) {
        return make_token(lexer, TOKEN_KW_RECORD, lexer->start, length);
    }
    ...
    if (length == 4 && strncmp(lexer->start, "Self", 4) == 0) {
        return make_token(lexer, TOKEN_KW_SELF, lexer->start, length);
    }
...
Token lexer_next(Lexer* lexer) {
    ...
    switch (lexer->start[0]) {
        ...
        case '~':
            return make_token(lexer, TOKEN_TILDE, lexer->start, 1);
        case '$':
