...
static void *run_cancel_child_wait(void *opaque) {
    (void)opaque;
...
void cancel_child_wait(JiangAsyncContinuation *continuation) {
    pending_continuation = continuation;
    pthread_create(&pending_thread, 0, run_cancel_child_wait, 0);
}

void cancel_child_wait_ready(void) {
    while (atomic_load(&gate_ready) == 0) {
        sched_yield();
    }
}

static void *open_cancel_child(void *opaque) {
    (void)opaque;
    usleep(10000);
...
void cancel_child_open_later(void) {
    pthread_t thread;
    if (pthread_create(&thread, 0, open_cancel_child, 0) == 0) {
        pthread_detach(thread);
    }
}

void cancel_child_join(void) {
