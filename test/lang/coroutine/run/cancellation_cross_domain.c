...
static void *run_cancellation_wait(void *opaque) {
    (void)opaque;
...
void cancellation_wait(JiangAsyncContinuation *continuation) {
    pending_continuation = continuation;
    pthread_create(&pending_thread, 0, run_cancellation_wait, 0);
}

void cancellation_wait_ready(void) {
    while (atomic_load(&gate_ready) == 0) {
        sched_yield();
    }
}

static void *open_cancellation(void *opaque) {
    (void)opaque;
    usleep(10000);
...
void cancellation_open_later(void) {
    pthread_t thread;
    if (pthread_create(&thread, 0, open_cancellation, 0) == 0) {
        pthread_detach(thread);
    }
}

void cancellation_join(void) {
